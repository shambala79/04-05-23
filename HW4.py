...
print(*sorted(set(n_array).intersection(m_array)))
...
garden_bed = []
N = (int(input('Введите количесво кустов на грядке:')))
for i in range (0, N):
    print(f"Куст {i+1} - ")
    garden_bed.append(int(input()))
max_harvest = 0
for i in range (0, len(garden_bed)):
    bush = garden_bed[i-1]+garden_bed[i-2]+garden_bed[i] 
    if max_harvest < bush:
       max_harvest = bush
    i +=1
print(max_harvest) 